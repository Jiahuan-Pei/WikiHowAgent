#!/bin/bash
#SBATCH --job-name=conversation_generation2
...
#SBATCH --gres=gpu:h100:1        # Request #number GPU, when you need more control over GPU type or specific features  (A100)
...
~/anaconda3/envs/worldtaskeval/bin/python main.py --processes $SLURM_CPUS_PER_TASK --job_id $SLURM_JOB_ID --batch_size 16 --config_teacher conf/ollama-openchat.yaml --config_learner conf/ollama-qwen2.yaml --config_evaluator conf/ollama-openchat.yaml
