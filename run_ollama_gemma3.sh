...
singularity exec --nv ollama_latest.sif ollama pull gemma3:12b
echo "Ollama model is downloaded!"
singularity exec --nv ollama_latest.sif ollama run gemma3:12b --verbose
echo "Test Ollama model inference time!"
time singularity exec --nv ollama_latest.sif ollama run gemma3:12b "Explain quantum mechanics in 100 words."
...
~/anaconda3/envs/worldtaskeval/bin/python main.py --processes $SLURM_CPUS_PER_TASK --job_id $SLURM_JOB_ID --batch_siz 128 --start_doc_id 145 --config_teacher conf/ollama-gemma3.yaml --config_learner conf/ollama-gemma3.yaml --config_evaluator conf/ollama-gemma3.yaml
