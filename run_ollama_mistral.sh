...
singularity exec --nv ollama_latest.sif ollama pull mistral
echo "Ollama model is downloaded!"
singularity exec --nv ollama_latest.sif ollama run mistral --verbose
echo "Test Ollama model inference time!"
time singularity exec --nv ollama_latest.sif ollama run mistral "Explain quantum mechanics in 100 words."
...
~/anaconda3/envs/worldtaskeval/bin/python main.py --processes $SLURM_CPUS_PER_TASK --batch_size 128 --start_doc_id 96 --config_teacher conf/ollama-mistral.yaml --config_learner conf/ollama-mistral.yaml --config_evaluator conf/ollama-mistral.yaml
