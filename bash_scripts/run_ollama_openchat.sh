...
# curl http://localhost:$PORT/api/generate -d '{"model": "openchat", "prompt": "Hello"}'

# --- 5. Run the Python Script with Correct Python Path ---
export PYTHONPATH=$PWD  # Ensure Python finds your package
~/anaconda3/envs/worldtaskeval/bin/python main.py --processes $SLURM_CPUS_PER_TASK --job_id $SLURM_JOB_ID --batch_size 128 --config_teacher conf/ollama-openchat.yaml --config_learner conf/ollama-openchat.yaml --config_evaluator conf/ollama-openchat.yaml
